...
Task Default -Depends Build,DbDropAndSetup,DbMigrate -Description "Cleans, Builds the application"
...
Task DbDropAndSetup -description "Drops and re-creates the database" {
 ...
 Task DbMigrate -Description "Migrates the database to the latest version" {
	 Exec {
		 ...
		 $migrate_exe = ".\packages\FluentMigrator.$rver\tools\migrate.exe" 
		 & $migrate_exe -a ".\db-migrator\bin\$Configuration\db-migrator.dll" -db="SqlServer" -conn="Integrated Security=SSPI;Persist Security Info=True;Initial Catalog=$dbname;Data Source=$dbserver;"
	 }
 } 